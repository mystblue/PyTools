...
DONCHIAN_ENTRY = 5           # エントリー用ドンチャン期間（高値）
DONCHIAN_EXIT = 5             # エグジット用ドンチャン期間（安値）
SMA_LONG = 50                # 長期トレンド判定用SMA
ATR_PERIOD = 14               # ATR期間
ATR_MULTIPLIER = 2            # ATRトレーリング倍率
...
for i in range(1, len(df)):
    ...
    # =========================
    # エグジット（ATRトレーリング or Donchian5割れ）
    # =========================
    ...
    # =========================
    # エントリー（SMA200上＆Donchian10高値ブレイク）
    # =========================
    if position is None:
...
if len(trades_df) == 0:
    ...
    print("===== 日本株向けプロ仕様 Donchian10 × SMA200 × ATRトレーリング =====")
    print(f"総トレード数      : {total_trレード数}")
