...
def add_indicators(df):
    ...
    df["Upper2"] = df["MA20"] + (df["STD20"] * 2)
    df["Lower2"] = df["MA20"] - (df["STD20"] * 2)
...
def is_buy_signal(df):
    ...
    upper = latest["Close"] > latest["Upper1"] and latest["Close"] < latest["Upper2"]
    prev_cond = prev["Close"] < prev["Upper1"]
    
    return upper and prev_cond
...
def main():
    ...
    if results:
        ...
        df_res.to_csv("screening_results_sigma1" + date_str + ".csv", index=False)
    else:
        print("今日の条件合致銘柄はなし")
