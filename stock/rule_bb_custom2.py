...
def add_indicators(df):
    ...
    df['TouchMid'] = (df['Low'] <= df['MA20']) & (df['High'] >= df['MA20'])
    
    df['Rebound'] = df['Close'] > df['Close'].shift(1)
...
def is_buy_signal(df):
    ...
    print(yesterday["TouchMid"])
    print(yesterday["Low"])
    print(yesterday["MA20"])
    middleTouch = yesterday['TouchMid']   # 前日にミドルタッチ
    rebound = latest['Rebound']               # 当日反発
    
    return middleTouch and rebound
