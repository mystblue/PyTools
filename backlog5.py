...
def main():
    ...
    issues = requests.get('https://pcidss.backlog.jp/api/v2/issues/PTG_DEV-409/comments?count=100&apiKey=' + 'FIVdvY4Chw9MbP2wph6Mybyb6aTf6LrgvlR99PugXy8OKbZYHPRQb4Qxnn2keSOf', headers = headers) # xxxxx は環境にあわせて設定

    #print(issues)
    #pprint.pprint(issues.json())
    #print(issues.json())
    with open("result_409.json", "w", encoding="utf-8") as f:
        f.write(json.dumps(issues.json()))

def main2():
    buf = ""
    with open("result_409.json", "r", encoding="utf-8") as f:
        buf = f.read()
