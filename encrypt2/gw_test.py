...
def get_key(sn):
    #seed = 'a949a6df9433d8cab20eb6f397e09540' #1000
    seed = 'eb9a6504a150cd2199282dd53035f183' #3000
    seed_use = seed[0:17]
    key_org = sn + seed_use
    key_orgs = key_org[0:32]
    return bytes.fromhex(key_orgs)

def create_request_param(sn, CHGSN, iv, request_param):
    ...
    print('・request = ' + request)
    enc = AES.new(key=get_key(CHGSN), mode=AES.MODE_CBC, iv=get_iv(iv))
...
def http_connect(sn, CHGSN, iv, request):
    headers = {'IV': iv, 'SN':sn, 'CHGSN':CHGSN, 'Sver': '3000', 'User-Agent': 'd&rCGr!%y9+A$LI4pSoG_$3dJUE39+H&=AG/ak*rxL$TqgQHO+-1bKSCalkBe4OB'}
...
if __name__ == '__main__':
    ...
    for psp in settings.psp_list:
        ...
        for request_param in request_param_list:
            ...
            request_param['CHGSN'] = term['CHGSN']
            ...
            request = create_request_param(term['sn'], term['CHGSN'], term['iv'], request_param)

            response = http_connect(term['sn'], term['CHGSN'], term['iv'], request)
            print("\nstatus code = " + str(response.status_code))
            print("\n・response = ")
            print(response.text)
            print(response.headers)
            
            print_response(term['CHGSN'], term['iv'], response)
