...
def crop_png_pil(src, dst, range):
    im = Image.open(src)
    im_crop = im.crop(range)
    im_crop.save(dst)



# 1080x1696+0+412 を (0, 412, 1080, 1696 + 412) に変換する
def convert_option(option):
    m = re.match('(\d+)x(\d+)\+(\d+)\+(\d+)', option)
    return (int(m.group(3)), int(m.group(4)), int(m.group(1)), int(m.group(4)) + int(m.group(2)))

def crop_png_inner(src, dst, option):
    range = convert_option(option)
    crop_png_pil(src, dst, range) #(0, 412, 1080, 1696 + 412)
    ...
    for option_name in folders:
        ...
        for folder_name in folders:
            ...
            for file in folders:
                ...
                crop_png_inner(file_path, dst_file_path, option)
